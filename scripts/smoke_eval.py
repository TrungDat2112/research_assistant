...
_REPORT_FILE = _OUT_DIR / "smoke_outputs.md"
_METRICS_FILE = _OUT_DIR / "smoke_metrics.json"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Smoke eval: five fixed seed queries (real APIs, costs money).",
    )
    p.add_argument(
        "--ab",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.ab:
        ...
        report_md = (
            ...
            f"Generated by `scripts/smoke_eval.py --ab`. "
    ...
    _REPORT_FILE.write_text(
        ...
        f"Generated from `scripts/smoke_eval.py` across {len(QUERIES)} queries.\n"
